...
fetch('folkets_en_sv_public.xdxf')
    .then(response => response.text())
    .then(function(xmlString) {
        lexicon = xml2js(xmlString, {'compact': true}).xdxf.lexicon.ar.sort(function(a, b) {
            var x = a.k._text,
                y = b.k._text;
        ...
        function findWord(word) {
            var i = lexicon.findIndex(obj => obj.k._text == word),
                wordList = [],
                nextWord;
            do {
                wordList.push(lexicon[i]);
                nextWord = lexicon[i + 1].k._text;
                i++;
            } while (nextWord === word);
            return wordList;
        }

        function showWords(wordsToShow) {
            if (wordsToShow) {
                document.getElementById('result').innerHTML = '';
                var words = t('ul.words');
                result.appendChild(t('h1', wordsToShow[0].k._text));
                ...
                for (var i = 0; i < wordsToShow.length; i++) {
                    ...
                    if (word.def.dtrn.constructor === Array) {
                        for (var ii = 0; ii < word.def.dtrn.length; ii++) {
                            translation.push(word.def.dtrn[ii]._text);
                        }
                    } else translation.push(word.def.dtrn._text);
 
                    words.appendChild(t('li.word', [
                        t('span.row1', group),
                        t('h2.row2', translation.join(', ')),
                        t('pre', JSON.stringify(word.def.ex, null, 4))
                    ]));
                }
            }
        }
        ...
        document.body.appendChild(
            t('div#wrapper', [
                t('div#header', t('h1', 'Quick English-Swedish Dictionary')),
                t('div#search.noinsert', [
                    t('input#word|placeholder=Type a word to look up|autofocus=autofocus|autocomplete=off', '', {'input': function() {
                        ...
                        if (this.value) {
                            ...
                            var wordList = lexicon.filter(function(el) {
                                return el.k._text.toLowerCase().replace('-', '').startsWith(this.value.toLowerCase());
                            ...
                            for (var i = 0; i < wordList.length; i++) {
                                var word = wordList[i].k._text,
                                    prevWord = wordList[i - 1] ? wordList[i - 1].k._text : '';
                                if (word !== prevWord) {
                                    filtered.appendChild(t('li', word, {'click': function() {
                                        document.getElementsByClassName('selected')[0].classList.remove('selected');
                                        this.classList.add('selected');
                                        showWords(findWord(this.innerHTML));
                                    }}));
                                }
                            }
                            ...
                            if (wordList.length) {
                                ...
                                showWords(findWord(wordList[0].k._text));
                                document.getElementById('filtered').scrollTop = 0;
                            } else {
                                result.innerHTML = '';
        ...
        document.body.classList.add(isTouch ? 'touch' : 'notouch');
            ...
            switch (e.keyCode) {
                case 40: // Down
                    ...
                    if (selected && selected.nextSibling) {
                        ...
                        showWords(findWord(selected.nextSibling.innerHTML));
                    }
                    break;
                case 38: // Up
                    e.preventDefault();
                    if (selected && selected.previousSibling) {
                        ...
                        showWords(findWord(selected.previousSibling.innerHTML));
                    }
                    break;
                case 27: // Escape
                    e.preventDefault();
